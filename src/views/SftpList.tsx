...
export function SftpList({ path, entries, error, onUp, onOpen, onClose }: SftpListProps) {
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg)" }}>
      <div style={header}>
        <button onClick={onUp} style={btn} aria-label="상위 폴더">⬆</button>
        ...
        <button onClick={onClose} style={btn} aria-label="파일 브라우저 닫기">×</button>
      </div>
      {error ? (
        <div style={{ padding: 16, color: "var(--st-danger)", fontSize: 13 }}>⚠ {error}</div>
      ) : entries.length === 0 ? (
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {entries.map((e) => (
            ...
            >
              <span style={{ width: 18 }}>{e.isDir ? "📁" : "📄"}</span>
...
const btn: CSSProperties = { border: "1px solid var(--border)", background: "var(--bg)", color: "var(--text2)", borderRadius: 5, padding: "3px 8px", fontSize: 12, cursor: "pointer" };
